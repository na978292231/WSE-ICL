...
def main():
    ...
    if "30" in args.llm_dir:
        model = AutoModelForCausalLM.from_pretrained(args.llm_dir, device_map='auto')
    ...
    for ins in tqdm(dev_data.data, total=dev_data.__len__()):
        ...
        running_time = running_time+es_time
    ...
    with open(save_results_file, 'a+', newline='') as csvfile:
        ...
        if not csv_exists:
            csvwriter.writerow(['dataset', 'llm', 'n_train_shot', 'seed', 'acc', 'running_time'])
