        ...
        with open(data_file, 'r') as f:
            ...
            for line in read_lines:
                if line!=""and line!="\n":
                    ...
                    for sentence,label in zip(instance['sentences'],instance['label']):
                        json_data = {'sentences':instance['sentences'],'sentence':sentence,'label':label}
                        
                        #instan = json.loads(json_data)
                        self.data.append(json_data)
        # customize your own label map in inheritance
        self.label2id = {0: 0, 1: 1}
        self.label2verb = {0: 'False', 1: 'True'}
...
class CLAUSEDataset(BASEDataset):
    def __init__(
        self,
        data_dir,
        mode
