def make_prompt(dataset, dataset_name, mode, indices=None):
    ...
    elif dataset_name == 'clause':
        template_func = template_clause
    elif dataset_name == 'wiki-zh':
        template_func = template_wikizh
    if mode == 'inference':
...
def template_sst2(ins, label, mode):
    if mode == 'train':
        ...
        return f"Review: {ins['sentence']}\nSentiment:"
def template_clause(ins, label, mode):
    if mode == 'train':
        return f"Sentence: {ins['sentence']}\nText segmentation boundary: {label}\n"
    else:
        return f"Sentence: {ins['sentence']}\nText segmentation boundary:"
def template_wikizh(ins, label, mode):
    if mode == 'train':
        return f"Sentence: {ins['sentence']}\nText segmentation boundary: {label}\n"
    else:
        return f"Sentence: {ins['sentence']}\nText segmentation boundary:"
...
def sent_sim_template(ins, dataset_name):
    ...
    if dataset_name in ['sst2', 'subj', 'mpqa', 'agnews', 'cr', 'dbpedia', 'mr', 'trec','clause','wiki-zh']:
        return ins['sentence']
